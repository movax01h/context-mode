...
// ═══════════════════════════════════════════════════════════
// 11. Knowledge-reuse event (ROI metric)
// ═══════════════════════════════════════════════════════════

describe("knowledge-reuse event", () => {
  test("timeline search writes knowledge-reuse event when prior session results found", () => {
    const store = createUnifiedStore();
    const sessionDB = createTestDB();
    const sessionId = `test-${randomUUID()}`;
    sessionDB.ensureSession(sessionId, "/project");
    sessionDB.insertEvent(sessionId, {
      type: "decision",
      category: "decision",
      data: "always push to next branch for releases",
      priority: 2,
    }, "PostToolUse", { projectDir: "/project", source: "test", confidence: 1 });

    searchAllSources({
      query: "always push to next",
      limit: 5,
      store,
      sort: "timeline",
      sessionDB,
    ...
    // knowledge-reuse event should have been written
    const events = sessionDB.getEvents(sessionId);
    const reuseEvents = events.filter((e: any) => e.category === "knowledge-reuse");
    expect(reuseEvents.length).toBe(1);
    expect(reuseEvents[0].type).toBe("search_hit_prior");
    expect(reuseEvents[0].data).toContain("prior sessions");
  });

  test("no knowledge-reuse event when no prior results found", () => {
    const store = createUnifiedStore();
    const sessionDB = createTestDB();
    const sessionId = `test-${randomUUID()}`;
    sessionDB.ensureSession(sessionId, "/project");
    ...
    const events = sessionDB.getEvents(sessionId);
    const reuseEvents = events.filter((e: any) => e.category === "knowledge-reuse");
    expect(reuseEvents.length).toBe(0);
  });

  test("no knowledge-reuse event without sessionId", () => {
    const store = createUnifiedStore();
    const sessionDB = createTestDB();
    const sessionId = `test-${randomUUID()}`;
    sessionDB.ensureSession(sessionId, "/project");
    sessionDB.insertEvent(sessionId, {
      type: "decision",
      category: "decision",
      data: "test decision for reuse",
      priority: 2,
    }, "PostToolUse", { projectDir: "/project", source: "test", confidence: 1 });

    searchAllSources({
      query: "test decision for reuse",
      limit: 5,
      store,
      sort: "timeline",
      sessionDB,
